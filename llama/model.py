...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # 
        # ipdb.set_trace()
        KVCacheManager.reserved_cache(xk,xv,seq,layer_id)
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # 需要把seq的信息转递过去,同时维持KV Cache
        h = x + self.attention(
            self.attention_norm(x), start_pos, freqs_cis, mask,seq,layers_id
        )
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor,seq:list[Request]):
        ...
        for idx,layer in enumerate(self.layers):
            h = layer(h, 0, freqs_cis, mask,seq,idx)
        h = self.norm(h)
